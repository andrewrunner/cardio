...
for ($i = 0; $i < $col_data + 1; $i++) { // линии вертикальные
    if ($i == 0) {
        $ri = -97;
        for ($nm = 18; $nm > 0; $nm--) { // линии горизонтальные
            $svg .= '<line fill="none" stroke="' . $color2 . '" stroke-width="' . $line . '" stroke-miterlimit="10" x1="-50" y1="' . $ri . '" x2="463" y2="' . $ri . '"/>';
            $ri += 11.2;
        }
    }
    if ($i > 0) {
...
for ($ty = 0; $ty < $rows; $ty++) { // запрос по каждому из методов в массиве
    ...
    for ($i = 0; $i < count(${'masres' . $mod[$ty]}); $i++) {
        if (${'masres' . $mod[$ty]}) {
            ...
            if ($admin) {
                $mnojitel = 'ик ' . ${'masres' . $mod[$ty]}[$i]['mnojitel'];
            } else {
                $mnojitel = '';
            }
            ...
            if ($cy != '') {                
                ...
                if ($g_cat == 'anketa' || $g_cat == 'crov' || $g_cat == 'ecg' || $g_cat == 'ad') {
                    $svg .= '<text class="text_svg" fill="#000" x="' . $cx . '" y="' . ($cc - 5) . '">' . ${'masres' . $mod[$ty]}[$i]['categories'] . '</text><text class="text_svg2" fill="#000" x="' . ($cx + 12) . '" y="' . ($cc - 5) . '">' . $mnojitel . '</text>';
                }
...
// формируем дату в таблице
for ($i = 0; $i < $col_data; $i++) {
    $svg .= '<div style="width: 37px; float: left; border-right: 1px solid #999; text-align: right;">' . ($i + 1) . '.02</div>';
}
